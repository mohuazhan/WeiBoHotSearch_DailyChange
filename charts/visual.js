...
function submitForm() {
  ...
  if (!myForm.token.value)
  {
    alert("请输入Token！");
    myForm.token.focus();
    return;
  }
  d3.select('svg').selectAll('*').remove(); //清空SVG中的内容
  var dict = '{"keyword":"hotsearch","platform":"weibo","start_date":"{0}","end_date":"{1}","token":"{2}"}';
  var param = String.format(dict, myForm.start_date.value, myForm.end_date.value, myForm.token.value);
  var param = JSON.parse(param); //由JSON字符串转换为JSON对象
  $.ajax({
    type:"post",
    url:"http://192.168.0.89:8089/v1/spider/hotsearch/weibo",
    data:JSON.stringify(param),
    dataType: "json",
    contentType: "application/json",
    success:function(respon){
      var data = respon['hotsearch_info'];
      data['columns'] = respon['columns'];
      draw(data);
      }
  });
};
